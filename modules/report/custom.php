...
$savedReports = $db->query(
    "SELECT * FROM saved_reports WHERE created_by = " . (int)$_SESSION['user_id'] . " ORDER BY created_at DESC"
    ...
    if ($action === 'generate') {
        ...
        if (!empty($selectedColumns)) {
            ...
            if (in_array('attendance', $selectedTables)) {
                $sql .= " LEFT JOIN attendance a ON e.employee_code = a.employee_code";
            }
            
            if (in_array('payroll', $selectedTables)) {
                $sql .= " LEFT JOIN payroll p ON e.employee_code = p.employee_code";
            }
