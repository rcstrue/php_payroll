...
function handleFileUpload($file, $uploadDir = 'uploads/employees/') {
    ...
    if (!in_array($file['type'], $allowedTypes)) {
        return ['error' => 'Invalid file type. Only JPG, PNG, GIF, PDF allowed.'];
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['profile_pic']['name'])) {
        $result = handleFileUpload($_FILES['profile_pic'], 'uploads/employees/photos/');
    ...
    if (!empty($_FILES['aadhaar_front']['name'])) {
        $result = handleFileUpload($_FILES['aadhaar_front'], 'uploads/employees/documents/');
        if (isset($result['path'])) {
    ...
    if (!empty($_FILES['aadhaar_back']['name'])) {
        $result = handleFileUpload($_FILES['aadhaar_back'], 'uploads/employees/documents/');
        if (isset($result['path'])) {
    ...
    if (!empty($_FILES['bank_document']['name'])) {
        $result = handleFileUpload($_FILES['bank_document'], 'uploads/employees/documents/');
        if (isset($result['path'])) {
                ...
                <div class="card-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <?php endif; ?>
                            <input type="file" class="form-control" name="bank_document" accept="image/*,.pdf">
                            <small class="text-muted">JPG, PNG, PDF (max 5MB)</small>
                        </div>
                    </div>
                </div>
