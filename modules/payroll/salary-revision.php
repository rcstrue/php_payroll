...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'single_revision') {
        ...
        $revisionMonth = (int)$_POST['revision_month'];
        $revisionYear = (int)$_POST['revision_year'];
        ...
        try {
            ...
            $db->insert('salary_revisions', [
                ...
                'percentage' => $revisionType === 'percentage' ? floatval($_POST['percentage']) : null,
                'daily_rate' => $revisionType === 'daily_rate' ? floatval($_POST['daily_rate']) : null,
                'effective_from' => $effectiveFrom,
    ...
    if ($action === 'bulk_revision') {
        ...
        $unitName = sanitize($_POST['unit_name'] ?? '');
        $revisionType = sanitize($_POST['bulk_revision_type']);
...
<div class="page-header">
    <div class="row align-items-center">
        <div class="col">
            ...
            <p class="text-muted">Manage salary increments, daily/monthly rates, and location changes</p>
        </div>
    </div>
</div>
...
<div class="tab-content">
            ...
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title mb-0"><i class="bi bi-pencil-square me-2"></i>Employee Salary Revision</h5>
                    </div>
                    <div class="card-body">
                        <form method="POST">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select select2" name="employee_id" required id="employeeSelect">
                                        ...
                                        <option value="<?php echo $emp['id']; ?>" 
                                                data-basic="<?php echo $emp['basic_wage']; ?>"
                                                data-gross="<?php echo $emp['gross_salary']; ?>">
    ...
    <div class="tab-pane fade" id="bulkRevision">
        <div class="card">
            <div class="card-header bg-warning">
                <h5 class="card-title mb-0"><i class="bi bi-people me-2"></i>Bulk Salary Revision</h5>
            </div>
            <div class="card-body">
                ...
                <form method="POST">
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <select class="form-select" name="client_name" id="bulkClient">
                                <option value="">All Clients</option>
                                <?php foreach ($clients as $c): ?>
                                <option value="<?php echo sanitize($c['client_name']); ?>">
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select" name="unit_name" id="bulkUnit">
                        ...
                        <div class="col-md-4">
                            <label class="form-label required">Increment %</label>
    ...
    <div class="tab-pane fade" id="history">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0"><i class="bi bi-clock-history me-2"></i>Revision History</h5>
            </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php elseif ($r['daily_rate']): ?>
                                    <span class="badge bg-info">Daily: <?php echo formatCurrency($r['daily_rate']); ?></span>
                                    <?php else: ?>
                                    <span class="badge bg-secondary">Fixed</span>
                                    <?php endif; ?>
                                </td>
                                <td class="text-end"><?php echo formatCurrency($r['old_gross']); ?></td>
                                <td class="text-end"><strong><?php echo formatCurrency($r['new_gross']); ?></strong></td>
                                <td class="text-success">
                                    <?php 
                                    $diff = $r['new_gross'] - $r['old_gross'];
...
$('#employeeSelect').on('change', function() {
    ...
    $('#currentGross').val('₹' + (opt.data('gross') || 0).toLocaleString());
});

// Calculate new gross
$('#newBasic, #newDA, #newHRA').on('input', function() {
    const basic = parseFloat($('#newBasic').val()) || 0;
    const da = parseFloat($('#newDA').val()) || 0;
    const hra = parseFloat($('#newHRA').val()) || 0;
    $('#newGross').val('₹' + (basic + da + hra).toLocaleString());
});

// Quick calculator
function calculateMonthly() {
    const daily = parseFloat($('#calcDailyRate').val()) || 0;
    const days = parseInt($('#calcDays').val()) || 26;
...
$('#bulkClient').on('change', function() {
    ...
    // This would require AJAX call to get count
});
</script>
