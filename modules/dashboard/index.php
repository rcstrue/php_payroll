...
$stmt = $db->query("SELECT e.employee_code, e.full_name, e.designation, e.unit_name, e.status, e.client_name 
                    FROM employees e 
                    WHERE e.status = 'approved'
                    ORDER BY e.created_at DESC LIMIT 5");
$recentEmployees = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Get unit-wise employee count
$stmt = $db->query("SELECT e.unit_name, COUNT(e.id) as count 
                    FROM employees e 
                    WHERE e.status = 'approved' AND e.unit_name IS NOT NULL AND e.unit_name != ''
                    GROUP BY e.unit_name
                    ORDER BY count DESC
                    LIMIT 10");
...
// Get client-wise employee count
$stmt = $db->query("SELECT e.client_name, COUNT(e.id) as count 
                    FROM employees e 
                    WHERE e.status = 'approved' AND e.client_name IS NOT NULL AND e.client_name != ''
