...
<body class="<?php echo $isLoggedIn ? '' : 'login-page'; ?>">
    ...
    <nav id="sidebar" class="sidebar">
       ...
       <div class="sidebar-footer">
        <?php
        $lastUpdate = '';
        $buildFile = dirname(__FILE__) . '/../build_time.txt';
        
        // Read build timestamp generated by GitHub Actions
        if (file_exists($buildFile)) {
            $lastUpdate = trim(file_get_contents($buildFile));
        }
        ...
        // Format display
        $displayDate = date('d M Y H:i', strtotime($lastUpdate));
        ?>
            <div class="sidebar-footer-version">Version 1.2.0</div>
            <div class="sidebar-footer-version" style="font-size:10px; opacity:0.8;">
                Last Update: <?php echo htmlspecialchars($displayDate); ?>
            </div>
        </div>
    </nav>
